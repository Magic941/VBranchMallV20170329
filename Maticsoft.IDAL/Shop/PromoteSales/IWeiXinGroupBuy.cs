...
namespace Maticsoft.IDAL.Shop.PromoteSales
{
    ...
    /// 接口层GroupBuy
    /// </summary>
    public interface IWeiXinGroupBuy
    {
        #region  成员方法
        /// <summary>
        /// 得到最大ID
        ...
        /// 增加一条数据
        /// </summary>
        int Add(Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy model);
        /// <summary>
        /// 更新一条数据
        /// </summary>
        bool Update(Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy model);
        /// <summary>
        /// 删除一条数据
        /// </summary>
        ...
        /// 得到一个对象实体
        /// </summary>
        Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy GetModel(int GroupBuyId);
        Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy DataRowToModel(DataRow row);
        // Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuyHistory DataRow2Model(DataRow row);
        /// <summary>
        /// 获得数据列表
        /// </summary>
        ...
        int BulkInsert2ShopGroup(List<Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy> GroupBuyList);
        DataSet GetCategory(string strWhere);//为了取分类数据
        DataSet GetGroupbyCategory(string strWhere);//为了取团购分类数据
        DataSet GetGroupBuyHot();
        Maticsoft.Model.Shop.PromoteSales.WeiXinGroupBuy GetPromotionLimitQu(int productId, int PromotionType = 1);
        ...
        /// 获取当前楼层当前排序是否存在数据
        /// </summary>
        DataSet GetFloor_IsDisplay(int FloorID, int Sequence, int IsIndex, int status, long ProductId);
        bool Update_status(int FloorID, int Sequence, int IsIndex, int status);
        
        //精品商城
        DataSet GetBoutiqueMall(string strWhere);
