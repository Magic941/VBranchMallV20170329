...
* Copyright © 2004-2012 Maticsoft. All Rights Reserved.
*/
;
(function () {
    var currenScript = $('[src$="maticsoft.GoodType.js"]').last(); //锁定脚本引用
    ...
    function InitLastSelect() {
        ...
        $.ajax({
            ...
            timeout: 10000,
            async: false,
            data: { Action: "GetWeixinParentNode", NodeId: $('[HFID=' + currentBaseSelectGuid + ']').val() },
    ...
    function InitFirstSelect() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            timeout: 10000,
            data: { Action: "GetWeixinDepthNode" },
            async: false,
            success: function (resultData) {
                switch (resultData.STATUS) {
                    case "OK":
                        $(resultData.DATA).each(function () {
                            baseSelect.append("<option value='" + this.GoodTypeID + "'>" + this.GoodTypeName + "</option>");
                        });
                        break;
                    default:
                        break;
    ...
    function LoadChildNodeData(send) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            timeout: 10000,
            data: { Action: "GetWeixinChildNode", ParentId: $(send).val() },
            async: false,
            success: function (resultData) {
                switch (resultData.STATUS) {
                    case "OK":
