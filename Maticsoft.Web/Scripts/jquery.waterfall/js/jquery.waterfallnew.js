...
; (function ($, window, document) {
	...
	$.fn.waterfall = function(options){
		...
		function getJSONData() {
				...
				if(colsHeight.minHeight + wf_box_top < $(window).height() + $(window).scrollTop()){
					...
					if(jsonCache.length > 0){
						...
						if(opts.ajaxTimes === 'infinite' || ajaxTimes < opts.ajaxTimes){
								...
								function (jsonData) {
								    ...
								    try{
								        if (jsonData.STATUS == "SUCCESS") {
								            if (typeof jsonData === 'string') jsonData = $.parseJSON(jsonData.DATA);
								            if ($.isEmptyObject(jsonData.DATA) || typeof jsonData === 'string') {
								                showMsg('finish');
								            } else {
							...
							opts.StartIndex=opts.StartIndex+1;
							opts.CurrentAjaxStartIndex = opts.StartIndex;
			...
			//if (opts.EndIndex <= opts.CurrentAjaxStartIndex)
			//{
			//    showMsg('finish');
			//}
		}
		...
		function dealData() {
			...
			loadImg(jsonCache, opts.imgUrlName,
                function () {
				while(perNum-- > 0 && (data = jsonCache.pop())){
					...
					htmlStr = createHtml(data);
					 ...
					 $wf_item = $('<div>').addClass(opts.itemClass).html(htmlStr).css({ width: opts.colWidth, left: wf_item_left, top: wf_item_top }).appendTo($wf_col);
					//opts.Obj.appendTo(htmlStr);
					$wf_img = $wf_item.find('.'+opts.imgClass);
					//$wf_img.height($wf_img.width() / data.width * data.height); 取消按宽缩小高度
					
					
					if (opts.isAnimation) {
					    $wf_item.css({ opacity: 0 }).animate({ opacity: 1 }, 800);
					}
	...
	$.fn.waterfall.defaults = {
	    Obj:null,
	    StartIndex: 1,	// 开始也索引
	    CurrentAjaxStartIndex: 1, //当前页索引
	    EndIndex:1,
		itemClass: 'wf_item',	// 砖块类名
		imgClass: 'thumb_img',	// 图片类名
		colWidth: 300,			// 列宽(int)
		marginLeft: 5,			// 每列的左间宽(int)
		marginTop: 5,			// 每列的上间宽(int)
		perNum: 'auto',			// 每次下拉时显示多少个(默认是列数)
		isAnimation: true,		// 是否使用动画效果
