...
namespace Maticsoft.Web.Areas.Shop.Controllers
{
    public class AccountController : Web.Controllers.ControllerBase
    {
        ...
        public ActionResult SendSMS(string phone, string SmsValCode)
        {
            ...
            string content = "尊敬的会 员,您健康商城的注册验证码为：" + rand.ToString() + ",请及时输入.";
        ...
        public ActionResult SendSMSForPwd(string phone, string Code)
        {
            ...
            Random rnd = new Random();
            int rand = rnd.Next(100000, 999999);
            string content = "您健康商城正在进行重置密码操作,验证码为：" + rand.ToString();
            Session["SMSCode"] = rand;
            Session["SMS_DATE"] = DateTime.Now;
            string[] numbers = new string[] { Phone };
            string errorMsg = "";
        ...
        private bool SendPwd(ref string randCode, string phone)
        {
            ...
            Random rnd = new Random();
            int rand = rnd.Next(100000, 999999);
            string content = "您正在进行找回密码操作，新密码为：" + rand.ToString() + "。如非本人操作请忽略【健康商城】：";
            Session["SMSCode"] = rand;
            Session["SMS_DATE"] = DateTime.Now;
            string[] numbers = new string[] { Phone };
            string errorMsg = "";
        ...
        public ActionResult Register(string id)
        {
            ...
            ViewBag.Description = pageSetting.Description;
            #endregion
            return View("PhoneRegisterNew",regModel);
        }
        ...
        public ActionResult PhoneRegisterNew()
        {
            return View();
        }
        ...
        public ActionResult ActiveSuccess()
        {
            ...
            {
                //模拟登录
                //Login(new LogOnModel() { UserName = userInfo.UserId, Password = userInfo.Password }, string.Empty);
            }
            else
            {
               // Login(new LogOnModel() { UserName = userInfo.UserId, Password = userInfo.Password }, string.Empty);
        ...
        #region OAuth.IApiBinding
        public interface ITaoLe : OAuth.IApiBinding
        {
        ...
        public class TaoLeServiceProvider : AbstractOAuth2ServiceProvider<ITaoLe>
        {
            public TaoLeServiceProvider(String clientId, String clientSecret)
                : base(new OAuth2Template(clientId, clientSecret,
                    "http://www.beitaichufang.com/api/v1/oauth2/authorize/",
                    "https://www.beitaichufang.com/api/v1/oauth2/token/", true))
            {
            }

            public override ITaoLe GetApi(AccessGrant accessGrant)
            {
                return new TaoLeTemplate(accessGrant);
            }
        }
        public class TaoLeOAuth2Template : OAuth2Template
        {
            public TaoLeOAuth2Template(string clientId, string clientSecret)
                : base(clientId, clientSecret,
                    "http://www.beitaichufang.com/api/v1/oauth2/authorize/",
                    "https://www.beitaichufang.com/api/v1/oauth2/token/",
                    false)
            {
            }
            ...
            protected override Task<AccessGrant> PostForAccessGrantAsync(string accessTokenUrl, NameValueCollection request)
            {
                return this.RestTemplate.PostForObjectAsync<NameValueCollection>(accessTokenUrl, request)
                    .ContinueWith<AccessGrant>(task =>
                    {
                        ...
                        return new AccessGrant(task.Result["access_token"], null, null, expires != null ? new Nullable<int>(Int32.Parse(expires)) : null, null);
                    }, TaskContinuationOptions.ExecuteSynchronously);
            }
        }
        public class TaoLeTemplate : AbstractOAuth2ApiBinding, ITaoLe
        {
            ...
            public TaoLeTemplate(AccessGrant accessGrant)
                : base(accessGrant)
            {
            }
        ...
        #region  健康商城卡激活 
        public ActionResult JKCard()
        {
            return View();
        }
        //协议
        public ActionResult JKAgreement()
        {
            return View();
        }
        //基本资料
        public ActionResult JKCardActive()
        {
            Shop_CardType cardType = Session["Shop_CardType"] as Shop_CardType;
            ...
            if (card != null)
            {
                ...
                ViewBag.Batch = card.Batch;
                ViewBag.SalesName = card.SalesName;
                ViewBag.CardNo = card.CardNo;
                ViewBag.CardSysId = card.Id;
                //未加密的密码
                ViewBag.Password = card.Password;
                ViewBag.JobType = cardType.JobTypeInsuranceCompanyCode;
            }
            if (cardType != null)
            {
                ViewBag.PersonNum = cardType.PersonNum;
        ...
        public ActionResult JKCardActiveConfirm()
        {
            Shop_CardUserInfo userinfo = Session["CardUserInfo"] as Shop_CardUserInfo;
            return View("JKCardActiveConfirm", userinfo);
        }

        //激活页面
        [HttpPost]
        public ActionResult JKCardActive(ShopCardUserInfo2 userinfo)
        {
            Shop_CardType cardType = Session["Shop_CardType"] as Shop_CardType;

            if (cardType.PersonNum >= 2)
            {
                //if (userinfo.NameOne == "")
                //{
                //    ModelState.AddModelError("NameOne", "第二投保人不能为空");
                //}
                //if (userinfo.NameOneCardId == "")
                //{
                //    ModelState.AddModelError("NameOne", "第二投保人身份证不能为空");
                //}
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                /*通过后转成另一个对象*/
                Shop_CardUserInfo userinfo2 = new Shop_CardUserInfo();
                //userinfo2.CardNo = userinfo.CardNo;
                ...
                //userinfo2.CardId = userinfo.CardId;
                //userinfo2.Email = userinfo.Email;
                //userinfo2.Moble = userinfo.Moble;
                //userinfo2.Address = userinfo.Address;
                //userinfo2.CodeNo = userinfo.CodeNo;
                //userinfo2.BirthDay = userinfo.BirthDay;
                ////userinfo2.IsMarry = userinfo.IsMarry;
                //userinfo2.Job = userinfo.Job;
                //userinfo2.BackPerson = userinfo.BackPerson;
                //userinfo2.CardSysId = userinfo.CardSysId;
                //userinfo2.NameOne = userinfo.NameOne;
                //userinfo2.NameOneCardId = userinfo.NameOneCardId;
                //userinfo2.RelationshipOne = userinfo.RelationshipOne;
                //userinfo2.BakPersonMoble = userinfo.BakPersonMoble;
                ViewBag.CardUserInfo = userinfo2;
                Session["CardUserInfo"] = userinfo2;
                return View("JKCardActiveConfirm", userinfo2);
                // return RedirectToAction("JKConfirmAgreement", "Account");

            }
            else { return JKCardActive(); }
        }

        /// <summary>
        /// 提交激活信息到激活协议页面
        /// 由于是异步，所以直接返回了视图，激活信息通过Session中传递
        /// </summary>
        /// <param name="userinfo"></param>
        /// <returns></returns>
        [HttpGet]
        public ActionResult JKConfirmAgreement()
        {
            Shop_CardType cardType = Session["Shop_CardType"] as Shop_CardType;
            ViewBag.Agreement = cardType.Agreement;
            ViewBag.ActivatePrompt = cardType.ActivatePrompt;
        ...
        public void JKCardUserInfo()
        {
            ...
            try
            {
                userInfo.ActiveDate = DateTime.Now;
                userInfo.CREATEDATE = DateTime.Now;
                Maticsoft.BLL.SysManage.ErrorLogTxt.GetInstance("注册激活日志").Write("  CardTypeNo:" + userInfo.CardTypeNo + ",Batch:" + card.Batch);
                ...
                if (userInfo.CardId == null)
                {
                   ...
                   // userInfo.Password = userInfo.S_Password;
                    //userInfo.S_Password = oldPassword; //保留未加密的密码
                    Instead = true;
                }
                else
                {
                ...
                JKActive(userInfo, card.Batch, Instead);
            }
            catch (Exception ex)
            {
        ...
        public void JKActive(Shop_CardUserInfo userInfo, string Batch, bool Instead = false)
        {
            UserCardLogic uc = new UserCardLogic();
            ...
            var one = new Insurants();
            one.Address = "sssss";
            one.BirthDay = DateTime.Now;
            one.CardId = "12345555555555555";
            one.CardNo = userInfo.CardNo;
            one.Email = "18885978@qq.com";
            one.Name = "吴少刚";

            userInfo.CardInsurantList.Add(one);
            ...
            {
                Maticsoft.BLL.SysManage.ErrorLogTxt.GetInstance("注册激活日志").Write("错误......" + ex.InnerException.Message);
                Response.Write("健康商城卡激活异常,请联系客服！错误信息：" + ex.Message + ",msg:" + ex);
            }
            try
            {
                string errorMsg = "";
                string content = "尊敬的" + userInfo.Name + "，恭喜您" + userInfo.CardNo + "会员卡激活成功,请登录 www.jk1699.com 查看个人信息及购物代金券";
                ...
                if (result == "1")
                {
                    ...
                    int userid = u.GetUserByName(userInfo.UserName).UserID;
                    Maticsoft.Common.ErrorLogTxt.GetInstance("线程调试日志").Write("主线程的id为:" + Thread.CurrentThread.ManagedThreadId);
                    //using (var releaser = m_lock.LockAsync())
                    //{
                    //    var task = Task.Run(() =>
                    //    {
                    //        Getcoupons(Batch, userid);
                    //    });
                    //}
        ...
        //根据规则自动发送优惠券
        public async void Getcoupons(string batchid, int userID)
        {
            Maticsoft.Common.ErrorLogTxt.GetInstance("线程调试日志").Write("coupons的线程id为:" + Thread.CurrentThread.ManagedThreadId);
            Maticsoft.BLL.Shop_CouponRuleExt cop = new Maticsoft.BLL.Shop_CouponRuleExt();
            var list = cop.GetListByBatchID(batchid);
            foreach (var item in list)
            {
                ...
                {
                    Maticsoft.Common.ErrorLogTxt.GetInstance("优惠券自动发送日志").Write("用户id: " + userID + "批次号为: " + batchid + "未发送成功");
                }
            }
            //  return 1;
        }
        ...
        //激活成功页面
        public ActionResult JKActiveSuccess()
        {
            var userInfo = Session["CardUserInfo"] as Shop_CardUserInfo;
            //if (userInfo.CardId == null)
            //{
            ...
            //else
            //{
           // Login(new LogOnModel() { UserName = userInfo.CardNo, Password = userInfo.S_Password }, string.Empty);
            //}
            return View();
            //var userInfo = Session["CardUserInfo"] as Shop_CardUserInfo;
            //if (userInfo != null)
            //{
            //    Maticsoft.Common.ErrorLogTxt.GetInstance("激活成功后登录日志").Write("身份证号为：" + userInfo.CardId == null ? "" : userInfo.CardId + ",登录用户名是：" + userInfo.UserName + ",密码是：" + userInfo.S_Password);
            //    //模拟登录   不管什么情况都是拿着卡号登录
