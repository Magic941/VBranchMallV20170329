...
namespace Maticsoft.Web.Areas.MShop.Controllers
{
    public class PayResultController : MShopControllerBase
    {
        ...
        public ActionResult Success(string viewName = "Success")
        {
            ...
            Maticsoft.Common.ErrorLogTxt.GetInstance("支付日志返回页面").Write("订单号为[" + orderIdStr + "]支付方式为：微信Success");
            string userid = "0";
            if (currentUser != null)
            {
        ...
        public ActionResult Fail(string viewName = "Fail")
        {
            string orderIdStr = Session[Handlers.Shop.Pay.PaymentReturnHandler.KEY_ORDERID] as string;
            string statusStr = Session[Handlers.Shop.Pay.PaymentReturnHandler.KEY_STATUS] as string;
            Maticsoft.Common.ErrorLogTxt.GetInstance("支付日志返回页面").Write("订单号为[" + orderIdStr + "]支付方式为：微信Fail");
