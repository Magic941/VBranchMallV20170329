...
$(function () {
    ...
    $('#btnEntryForm').die().live('click', function () {
        ...
        if (phone.length > 0) {
            if (!/^1([34578][0-9]|4[57]|5[^4])\d{8}$/.test(phone)) {
        ...
        $.ajax({
           // beforeSend: function () { $.mobile.showPageLoadingMsg(); }, //显示加载器
            url: $Maticsoft.BasePath + "UserCenter/SubmitEntryForm",
            type: 'post',
            dataType: 'text',
            timeout: 10000,
            async: false,
            ...
            success: function (JsonData) {
               // $.mobile.hidePageLoadingMsg(); //隐藏加载器  
                switch (JsonData) {
                    case "isnotnull":
                        ShowFailTip("您已经申请过健康服务店，请不要重复申请！");
                        break;
                    case "UserNameISNULL":
                        ShowFailTip(" 请填写名称！");
                        break;
                    case "true":
                        ...
                        ShowSuccessTip(" 提交成功！3秒后自动返回！");
                        window.setTimeout("window.location='/MShop/u/ShopGoodDistribution'", 3000);
                        break;
                    default:
