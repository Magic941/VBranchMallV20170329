    ...
    public partial class EditPaymentMode : PageBaseAdmin
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.Page.IsPostBack)
            {
                ...
                {
                    PaymentModeInfo paymentMode = PaymentModeManage.GetPaymentModeById(Maticsoft.Common.Globals.SafeInt(this.Page.Request.QueryString["modeId"], 0));
                    if (paymentMode == null)
                    {
                ...
                if (this.item != null)
                {
                    ...
                    this.DisplayControls();
                    this.txtName.Text = Maticsoft.Common.Globals.HtmlDecode(this.item.Name);
                    ...
                    {
                        this.txtEmailAddress.Text = Maticsoft.Common.Globals.HtmlDecode(this.item.EmailAddress);
                    }
                    ...
                    {
                        this.txtPartner.Text = Maticsoft.Common.Globals.HtmlDecode(this.item.Partner);
                    }
        ...
        public new bool IsValid
        {
            get
            {
                ...
                if (this.tblCharge.Visible && (Maticsoft.Common.Globals.SafeDecimal(this.txtCharge.Text, -1M) <= -1M))
                {
        ...
        public PaymentModeInfo Item
        {
            get
            {
                ...
                {
                    ModeId = Maticsoft.Common.Globals.SafeInt(this.Page.Request.QueryString["modeId"], -1),
                    MerchantCode = this.txtMerchantCode.Text.Trim(),
                    EmailAddress = Maticsoft.Common.Globals.HtmlEncode(this.txtEmailAddress.Text.Trim()),
                    SecretKey = this.txtSecretKey.Text.Trim(),
                    SecondKey = this.txtSecondKey.Text.Trim(),
                    Password = this.txtPassword.Text.Trim(),
                    Partner = Maticsoft.Common.Globals.HtmlEncode(this.txtPartner.Text.Trim()),
                    Name = Maticsoft.Common.Globals.HtmlEncode(this.txtName.Text.Trim()),
                    Description = this.fcContent.Value.Replace("\r\n", "").Replace("\r", "").Replace("\n", ""),
                    Gateway = this.dropPayInterface.SelectedValue.ToLower(),
                    DisplaySequence = Maticsoft.Common.Globals.SafeInt(this.txtDisplaySequence.Text.Trim(), -1),
                    AllowRecharge = this.radAllowRecharge.SelectedValue,
                    DrivePath = drivePath
                };
                if ((provider.Name != "advanceaccount") && (provider.Name != "bank"))
                {
                    info.Charge = Maticsoft.Common.Globals.SafeDecimal(this.txtCharge.Text.Trim(), 0M);
