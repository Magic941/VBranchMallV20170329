...
namespace Maticsoft.Web.Admin.WeChat.ShopCategories
{
    public partial class List : PageBaseAdmin
    {
        ...
        protected new int Act_AddData = 252;    //CMS_图片分类管理_添加数据
        protected new int Act_UpdateData = 253;    //CMS_图片分类管理_编辑数据
        protected new int Act_DelData = 254;    //CMS_图片分类管理_删除数据

        readonly BLL.Shop.Products.GoodsType goodstypebll = new BLL.Shop.Products.GoodsType();
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (goodstypebll.DeleteList(idlist))
            {
        ...
        public void BindData()
        {
            ...
            {      
                strWhere.AppendFormat("GoodTypeName like '%{0}%'", txtKeyword.Text.Trim());
            }
            ds = goodstypebll.GetList(strWhere.ToString());
        ...
        public string GetParentName(object obj)
        {
            ...
            if (obj != null && obj.ToString() != "0" && PageValidate.IsNumber(obj.ToString()))
            {
                ...
                Model.Shop.Products.GoodsType goodsmodel = goodsbll.GetModel(int.Parse(str));
                if (goodsmodel != null)
                {
                    str = goodsmodel.GoodTypeName;
                }
            }
            return str;
        }
