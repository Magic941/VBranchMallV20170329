...
namespace Maticsoft.Web.Admin.Shop.ProductType
{
    public partial class ListSpec : PageBaseAdmin
    {
        ...
        protected new int Act_AddData = 509;    //Shop_规格值_添加数据
        protected new int Act_UpdateData = 510;    //Shop_规格值_编辑数据
        protected new int Act_DelData = 511;    //Shop_规格值_删除数据
        //int Act_ShowInvalid = -1; //查看失效数据行为
        Maticsoft.BLL.Shop.Products.AttributeValue bll = new Maticsoft.BLL.Shop.Products.AttributeValue();
        ...
        private void ShowInfo()
        {
            ...
            Maticsoft.Model.Shop.Products.AttributeInfo model = bll.GetModel(AttId);
            if (model != null)
            {
        ...
        protected string SpecShow(object valueStr,object imgStr)
        {
            if (IsImg == 1)
            {
                ...
                {
                    return "<img src='" + Maticsoft.Web.Components.FileHelper.GetImageUrl(imgStr.ToString()) +"' title='" + valueStr.ToString() + "' Width='23' height='20'/>";
                }
                else
                {
                    return "";
