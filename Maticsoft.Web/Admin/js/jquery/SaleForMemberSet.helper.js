...
function AddSelectedSKUId(skuid, thisEvent) {
    ...
    var temp = $('[id$=hfSelectedData]').val();
    $('[id$=hfSelectedData]').val(temp + "," + currentTable.attr('skuid'))

    //var tmpSKU = parentSelectedAccessories.val() + skuid + ',';
    parentSelectedAccessories.val($('[id$=hfSelectedData]').val);
...
function DelSelectedSKUId(skuid, thisEvent) {
    ...
    var skus = $('[id$=hfSelectedData]').val().split(','); //parentSelectedAccessories.val().split(',');
    if (skus) {
        //delete skuid
