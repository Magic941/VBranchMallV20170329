...
$(document).ready(function () {
    ...
    $('.AMsubmit_add').bind('click', function () {
        InitAddProduct(this, addedproductslist);
        //添加
        var currentTableTR = $(this).parents('tr:last');
        var currentTable = currentTableTR.find('table');
        var ProductId = currentTable.attr('ProductId');
        var ProductName = currentTable.attr("ProductName");
        AddAMProduct(ProductId, $.getUrlParam("AMId"), ProductName);
    });
    $('.AMsubmit_del').bind('click', function () {
        InitDelProduct(this, searchproductslist);
        //删除
        var ProductId = $(this).attr('ProductId');
        DeleteAMProduct(ProductId, $.getUrlParam('AMId'));
    });
...
//添加
function AddAMProduct(ProductId, AMId, ProductName) {
    $.ajax({
        url: "/ShopManage.aspx",
        type: 'POST', dataType: 'json', timeout: 10000,
        data: { Action: "AddAMProduct", Callback: "true", ProductId: ProductId, AMId: AMId, ProductName: ProductName },
        async: false,
...
//删除
function DeleteAMProduct(ProductId, AMId) {
    $.ajax({
        url: "/ShopManage.aspx", //web.config 配置调用 Web-Handlers-shop-ShopHandlers
        type: 'POST', dataType: 'json', timeout: 10000,
        data: { Action: "DeleteAMProduct", Callback: "true", ProductId: ProductId, AMId: AMId },
        async: false,
        success: function (resultData) {
...
function InitDelProduct(send, targetContext) {
    ...
    targetTableTR.find('.AMsubmit_del').removeClass('AMsubmit_del').addClass('AMsubmit_add').text('添加').bind('click', function () {
        InitAddProduct(this, addedproductslist);
    });
...
function InitAddProduct(send, targetContext) {
    ...
    targetTableTR.find('.AMsubmit_add').removeClass('AMsubmit_add').addClass('AMsubmit_del').text('删除').bind('click', function () {
        InitDelProduct(this, searchproductslist);
    });
