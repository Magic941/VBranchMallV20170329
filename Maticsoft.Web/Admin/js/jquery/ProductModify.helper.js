...
var skusTableHeadThBase = '<th style="width:30%;"><em>*</em>商品编码</th><th style="width:90px;"><em>*</em>销售价</th><th style="width:90px">成本价</th><th style="width:90px">内部成本</th>';  ///////////////////3-19修改
...
skusTableTdBase += '<td ><input value="{3}" id="CostPrice2" class="OnlyFloat" style="width:90px;text-align: right;" maxlength="9 type="text"/></td>';    ///////////////////3-19修改
skusTableTdBase += ' <td ><input value="{4}" id="Stock" class="OnlyNum" style="width:40px;" maxlength="5" type="text"/></td>';
skusTableTdBase += '<td ><input value="{5}" id="AlertStock" class="OnlyNum" style="width:40px;" maxlength="5" type="text"/></td>';
skusTableTdBase += '<td ><input value="{6}" id="Weight" class="OnlyNum" style="width:40px;" maxlength="10" type="text"/></td>';
skusTableTdBase += '<td ><input {7} id="Upselling" type="checkbox"/></td>';
                ...
                case "SUCCESS":
                    $(resultData.DATA).each(function () {
                        ...
                        if (this.ThumbnailUrl1) {
                           // alert(this.ThumbnailUrl1);
                            $('[id$=hfImage0]').val(picServerUrl+"/"+this.ThumbnailUrl1.replace('T150X150_', "{0}"));
                        }
...
function GeneratedImageInfo() {
    $.ajax({
        ...
        success: function (resultData) {
            var picServerUrl = resultData.PicServerUrl;
            var picName = "T158X158_";
            switch (resultData.STATUS) {
                case "SUCCESS":
                    $(resultData.DATA).each(function (index) {

                        if (index == 0) {                           
                            $('[id$=hfImage1]').val(picServerUrl + "/" + this.ThumbnailUrl1.replace(picName, "{0}"));
                        }
                        if (index == 1) {
                            $('[id$=hfImage2]').val(picServerUrl + "/" + this.ThumbnailUrl1.replace(picName, "{0}"));
                        }
                        if (index == 2) {
                            $('[id$=hfImage3]').val(picServerUrl + "/" + this.ThumbnailUrl1.replace(picName, "{0}"));
                        }
                        if (index == 3) {
                            $('[id$=hfImage4]').val(picServerUrl + "/" + this.ThumbnailUrl1.replace(picName, "{0}"));
                        }
                    });
                    break;
                default:
                        ...
                        $("#" + this.AttributeId).find("[name='AttributeValue" + this.AttributeId + "']").each(function (index) {
                            if (selectAttributeValueId == this.value) {
                                ...
                                //if (isDefineImage) {
                                    ...
                                    $('#color_' + CurrentAttribute).find('tr').each(function () {
                                        if (selectAttributeValueId == this.id) {
                                            ...
                                            if (CurrentImage!=null) {
                                                $('#imgURL_' + this.id).attr("src", picserverurl + CurrentImage.format('T32X32_'));
                                                $('#imgURL_' + this.id).attr("value",picserverurl+ CurrentImage);
                                            }
                                    ...
                                    $('#attrValueText_' + this.value).blur(function () {
                                        $($("#contetSKUs").find(".edit")).each(function () {
                                            $(".SkuLabel_" + $(this).attr('vid')).text($("#attrValueText_" + $(this).attr('vid')).val());
                                        });
                                    });
                                //}
                            }
...
function GeneralHavnSKUItem(attributeIds, htSkuData, skuArray) {
    ...
    $(skuArray).each(function (index) {
        ...
        var strSpecFormat = "{0}_{1}";
        var strDivider = "|";
        var strSpecs = "";
        $(this).each(function () {
            dataTR.append(skusTableTd.format(this.AttributeId, this.ValueId, this.ValueId, this.Value));
            strSpecs += (strSpecs == "" ? strSpecs : strDivider) + strSpecFormat.format(this.AttributeId, this.ValueId);
        });
        $.ajax({
            url: '/ShopManage.aspx',
            type: 'POST',
            ...
            timeout: 10000,
            async: false,
            data: { Action: "ProductSkusInfo", pid: currentPid, specs: strSpecs },
            success: function (resultData) {
                switch (resultData.STATUS) {
                    case "SUCCESS":
                        if (resultData.DATA.length > 0) {
                            dataTR.append(skusTableTdBase.format(
                                        !resultData.DATA[0].SKU ? "" : resultData.DATA[0].SKU,
                                        resultData.DATA[0].SalePrice > 0 ? parseFloat(resultData.DATA[0].SalePrice).toFixed(2) : (0.00).toFixed(2),
                                        resultData.DATA[0].CostPrice > 0 ? parseFloat(resultData.DATA[0].CostPrice).toFixed(2) : (0.00).toFixed(2),
                                        resultData.DATA[0].CostPrice2 > 0 ? parseFloat(resultData.DATA[0].CostPrice2).toFixed(2) : (0.00).toFixed(2),  ///////////////////3-19修改
                                        resultData.DATA[0].Stock > 0 ? resultData.DATA[0].Stock : 0,
                                        resultData.DATA[0].AlertStock > 0 ? resultData.DATA[0].AlertStock : 0,
                                        resultData.DATA[0].Weight > 0 ? resultData.DATA[0].Weight : 0,
                                        resultData.DATA[0].Upselling ? "checked='checked'" : ""));
                        }
                        break;
                    default:
                        break;
...
function LoadCheckResult(senderevent, skucode) {
    $.ajax({
        ...
        success: function (resultData) {
            ...
            alert(xmlHttpRequest.responseText+"错2");
        }
    });
            ...
            if (!$(this).val()) {
                ...
                $.ajax({
                    ...
                    success: function (resultData) {
                        ...
                        alert(xmlHttpRequest.responseText+"错3");
                    }
                });
        ...
        success: function (resultData) {
            switch (resultData.STATUS) {
                case "SUCCESS":
                    ...
                    //方便搜索指定的分类 wusg 20140718
                    allProductTypesData = resultData.DATA;
                    $(resultData.DATA).each(function () {
                            ...
                            $(data.AttributeValues).each(function () {
                                ...
                                targetInput.click(function () {
                                    ...
                                    if ($(this).attr('checked')) {
                                        $("#color_" + this.attrid).attr("style", "");
                                        $(".colorclass").attr("style", "");
                                        $("#attrValuelabel_" + this.value).attr("style", "display: none;");
                                        $("#attrValueText_" + this.value).attr("style", "");

                                        $("#color_" + this.attrid).attr("style", "");
                                        $("#" + this.value).attr("style", "");
                                        $("#selectValueLabel_" + this.value).attr("title", $("#attrValueText_" + this.value).val()).text($("#attrValueText_" + this.value).val());
                                        $(".SkuLabel_" + this.value).text($("#attrValueText_" + this.value).val());

                                        if (data.UserDefinedPic) {
                                            $("#" + this.value).show();
                                            $("#selectValueLabel_" + this.value).text($("#attrValueText_" + this.value).val());
                                        }
                                    } else {
                                        $("#color_" + this.attrid + " #" + this.value).attr("style", "display: none;");
                                        $("#attrValuelabel_" + this.value).attr("style", "");
                                        $("#attrValueText_" + this.value).attr("style", "display: none;");
                                        $("#" + this.value).attr("style", "display: none;");
                                    }
                                    if ($(this.attrid).find('input:checked').length <= 0) {
                                        $("#color_" + this.attrid).show();
                                        $(".colorclass").show();
                                    }
                                    $("#attrValueText_" + ve).blur(function () {
                                        $($("#contetSKUs").find(".edit")).each(function () {
                                            $(".SkuLabel_" + $(this).attr('vid')).text($("#attrValueText_" + $(this).attr('vid')).val());
                                        });
                                    });

                                    $("#attrValueText_" + ve).unbind("click");
                                });
    ...
    for (var j = 0; j < skuArray.length; j++) {
        ...
        (function (lockedInIndex) {
            ...
            dataTR.append(tmpTableTdBase.format(
                        ...
                        parseFloat(productSKU.CostPrice2).toFixed(2), ///////////////////3-19修改
                        productSKU.Stock,
...
function GetBaseSKU() {
    var txtProductSKU, txtSalePrice, txtCostPrice, txtCostPrice2, txtStock, txtAlertStock, txtWeight, chkUpselling;
    ...
    if ($('[id$=txtCostPrice2]').val()) {
        txtCostPrice2 = parseFloat($('[id$=txtCostPrice2]').val());
    } else {
        txtCostPrice2 = 0;
    }
    if ($('[id$=txtStock]').val()) {
    ...
    var res = { "SKU": txtProductSKU,
        ...
        "CostPrice2": txtCostPrice2 ? txtCostPrice2 : 0,
        "Stock": txtStock,
...
function GetSelectedSKU() {
    ...
    SKUItemTR.each(function () {
        ...
        txtCostPrice2 = $(this).find('#CostPrice2').val();
        ...
        SKUInfos.push({
            ...
            "CostPrice2": txtCostPrice2 ? parseFloat(txtCostPrice2) : "",
...
//商品类型选择方法
function SelectProductTypeKeyPress() {
    var inputValue = $("#txtProductTypeSearch").val();
    // alert(allProductTypesData.length);
    if (allProductTypesData.length > 0) {
        var result = $.grep(allProductTypesData, function (item, i) {
            //alert(item.TypeName.indexOf(inputValue) >= 0);
            return item.TypeName.indexOf(inputValue) >= 0;
        });
