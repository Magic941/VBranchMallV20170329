...
{
    public partial class AMPDAL :IAMP
    {
        public AMPDAL()
		{}
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select AMId,AMName,ProductId,ProductName,ProductCode,GiftsProId,GiftsProName ");
...
ON a.ProductId = b.productid ");
        ...
        public Maticsoft.Model.Shop.ActivityManage.AMPModel DataRow2ToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["GiftsProName"] != null)
                {
                    model.GiftsProName = row["GiftsProName"].ToString();
                }
            }
            return model;
        }
        ...
        public DataSet GetListAndPrice(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(@" a.AMPId,a.AMId,a.ProductId,a.ProductName,a.SupplierId,GiftsProId,GiftsProName,CONVERT(DECIMAL(12,2),b.LowestSalePrice) LowestSalePrice FROM dbo.Shop_ActivityManageProduct a 
INNER JOIN (SELECT * FROM dbo.Shop_Products) b 
ON a.ProductId = b.productid  ");
            if (strWhere.Trim() != "")
            {
