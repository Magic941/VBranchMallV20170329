...
namespace Maticsoft.SQLServerDAL.Shop.Order
{
    ...
    public partial class OrderAction : IOrderAction
    {
        ...
        public Maticsoft.Model.Shop.Order.OrderAction GetModel(long ActionId)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ActionId,OrderId,OrderCode,UserId,Username,ActionCode,ActionDate,Remark from Shop_OrderAction ");
            strSql.Append(" where ActionId=@ActionId");
            SqlParameter[] parameters = {
					new SqlParameter("@ActionId", SqlDbType.BigInt)
			};
        ...
        #region 查询已发货7天未完成订单
        public DataSet GetNoCompleteOrder()
        {
            StringBuilder stb = new StringBuilder();
            stb.Append("SELECT b.OrderId,ActionDate FROM dbo.Shop_Orders a INNER JOIN dbo.Shop_OrderAction b ON a.OrderCode = b.OrderCode WHERE b.ActionCode=104");
            stb.Append(" AND a.ShippingStatus<>3 AND a.OrderStatus<>2 GROUP BY b.OrderId,b.ActionDate HAVING DATEDIFF(dd,ActionDate,GETDATE())>7 ");
        ...
        public Maticsoft.Model.Shop.Order.OrderAction DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                if (row.Table.Columns.Contains("ActionId"))
                {
                ...
                if (row.Table.Columns.Contains("OrderCode"))
                {
                ...
                if (row.Table.Columns.Contains("ActionCode"))
                {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from Shop_OrderAction T ");
