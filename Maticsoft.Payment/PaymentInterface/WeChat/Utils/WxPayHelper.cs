namespace Maticsoft.Payment.PaymentInterface.WeChat.Utils
{
    ...
    public class WxPayHelper
    {
        ...
        public string GetBizMD5Sign(Dictionary<string, string> bizObj)
        {
            string bizString = CommonUtil.FormatBizQueryParaMapForUnifiedPay(bizObj);
        ...
        public string CreateBizPackage()
        {
            ...
            var entries = nativeObj.Select(d => string.Format("\"{0}\": \"{1}\"", d.Key, d.Value));
            Maticsoft.Common.ErrorLogTxt.GetInstance("微信支付生成jsapi支付请求json日志返回页面").Write("{" + string.Join(",", entries.ToArray()) + "}");
            return "{" + string.Join(",", entries.ToArray()) + "}";

        }

        public string CreateBizPackage(string prepayid)
        {
            Dictionary<string, string> nativeObj = new Dictionary<string, string>();
            ...
            nativeObj.Add("appId", AppId);
            nativeObj.Add("package", string.Format("prepay_id={0}", prepayid));
            nativeObj.Add("timeStamp", ((DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000000).ToString());
            nativeObj.Add("nonceStr", CommonUtil.CreateNoncestr());
            nativeObj.Add("signType", SignType);
            nativeObj.Add("paySign", GetBizMD5Sign(nativeObj));
