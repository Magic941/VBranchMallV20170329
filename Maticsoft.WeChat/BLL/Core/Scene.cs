...
namespace Maticsoft.WeChat.BLL.Core
{
	...
	{
        private readonly IScene dal = Maticsoft.DBUtility.PubConstant.IsSQLServer ? (IScene)new WeChat.SQLServerDAL.Core.Scene() : null;
		public Scene()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Maticsoft.WeChat.Model.Core.Scene GetModelByCache(int SceneId)
		{
			...
			return (Maticsoft.WeChat.Model.Core.Scene)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Maticsoft.WeChat.Model.Core.Scene> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Maticsoft.WeChat.Model.Core.Scene> DataTableToList(DataTable dt)
		{
			List<Maticsoft.WeChat.Model.Core.Scene> modelList = new List<Maticsoft.WeChat.Model.Core.Scene>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Maticsoft.WeChat.Model.Core.Scene model;
